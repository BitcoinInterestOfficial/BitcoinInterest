...
result progpow(const epoch_context& context, const hash256& header_hash, uint64_t nonce) noexcept
{
    ...
    hash256 ret_hash = ethash_keccak256(header_hash, seed, &mix_hash.hwords[0]);
    hash256 final_hash;
    for (int i = 0; i < 8; i ++) {
        final_hash.hwords[i] = bswap32(ret_hash.hwords[i]);
    }
...
ethash_hash256 verify_final_progpow_hash(const hash256& header_hash, const hash256& mix_hash, uint64_t nonce) noexcept
{
    ...
    for (int i = 0; i < 4; i++)
        result[i] = 0;
    uint64_t seed = keccak_f800(header_hash, nonce, result);
    hash256 h = ethash_keccak256(header_hash, seed, &mix_hash.hwords[0]);
    hash256 final_hash;
...
bool verify_final_progpow(const hash256& header_hash, const hash256& mix_hash, uint64_t nonce,
    const hash256& boundary) noexcept
{
    hash256 calc_hash = verify_final_progpow_hash(header_hash, mix_hash, nonce);

    return is_less_or_equal(calc_hash, boundary);
}
...
{
    if(!verify_final_progpow(header_hash, 
                             mix_hash, nonce, boundary)) {
        return false;
    } 
    ...
    const hash256& boundary, uint64_t nonce) noexcept
{
    ...
    return {false, r};
        //return std::tuple <true, r>;
    //return std::tuple <false, r>;
}
