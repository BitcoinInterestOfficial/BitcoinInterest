...
public:
    CMainParams() {
        ...
        //based on https://github.com/BitcoinInterest1/BitcoinInterest/issues/78
        ...
        vSeeds.emplace_back("eu-dnsseed.bitcoininterest-official.org", true);
        vSeeds.emplace_back("dnsseed.bitcoininterest.org", true);
        vSeeds.emplace_back("dnsseed.BTCINT.org", true);
...
public:
    CTestNetParams() {
        ...
        vSeeds.emplace_back("eu-test-dnsseed.bitcoininterest-official.org", true);
        vSeeds.emplace_back("test-dnsseed.bitcoininterest.org", true);
        vSeeds.emplace_back("test-dnsseed.BTCINT.org", true);
        vSeeds.emplace_back("BCI.dnsseed.minertopia.org", true);
...
bool CChainParams::IsPremineAddressScript(const CScript& scriptPubKey, uint32_t height) const {
    ...
    assert((uint32_t)consensus.BCIHeight <= height &&
           height < (uint32_t)(consensus.BCIHeight + consensus.BCIPremineWindow));
    if (!consensus.BCIPremineEnforceWhitelist) {
        return true;
    }
    int block = height - consensus.BCIHeight;
    int num_unlocked = consensus.BCIPremineWindow * 40 / 100;  // 40% unlocked.
    int num_locked = consensus.BCIPremineWindow - num_unlocked;  // 60% time-locked.
    ...
    if (block < num_unlocked) {
        ...
        int lock_time = consensus.BCIHeight + stage_lock_time * (1 + stage);
